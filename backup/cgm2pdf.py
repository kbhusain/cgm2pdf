...
import io, os,sys , getopt, array, Image
from struct import *
from cgmdesc import *
from pPDFout import *
from pSVGout import *
...
class cgmParameters:
	def __init__(self,os_prelude):
		...
		self.parameters['pattern_index'] =  1
...
class cgmFileReader:
	...
	def readFile(self,inputfilename,outputfile,xx,svg=0,writeit=1):
		if svg == 0: 
			self.pdfOut = PDFout(outputfile); 
		else: 
			self.pdfOut = SVGout(outputfile); 
		fd = open (inputfilename,'rb')
		self.myrecords = [ ]
		print "Opening", svg, inputfilename , fd.tell() 
		i = 0
		for i in range(1000000): 
			mb = cgmRecord(self.usedefaults,i)      # keep these in an arra
			self.myrecords.append(mb)
			r = xx.readRecord(fd,mb)
			#r = self.readRecord(fd,mb)
			if r == 0: break;
			i += 1;
	...
	def c4_01(self,record,fd): 
		...
		patternIndex  =self.usedefaults.parameters.get('pattern_index',1) 
		...
		#print "C4_01 :", sw, len(upoints), fill_style, color_line, color_fill, color_edge 
		self.pdfOut.addPolyLine(polygon=0, stroke=color_line, strokewidth=sw, points=upoints,	edge_visible = edge_visible, 
				color_fill = "none",  strokedasharray=self.usedefaults.parameters['stroke_dasharray'],patternIndex=str(patternIndex));
		#print "C4_01 : Done  ", sw, len(upoints), fill_style, color_line, color_fill, color_edge 
	...
	def c4_04(self,record,fd): 
		# print "where", fd.tell(), self.usedefaults.os_prelude, self.usedefaults.fmt_vbpString 
		self.dummyReader(fd,record)
		...
		# ----------------------------------------------------------------------------------------
		self.fontsize = fsz * 5   
		self.pdfOut.addTextTag(x,y,s,self.usedefaults,anchor=self.usedefaults.parameters['text_anchor'], 
	...
	def c4_07(self,record,fd): 
		...
		edge_visible=self.usedefaults.parameters['edge_visible'] 
		patternIndex  =self.usedefaults.parameters.get('pattern_index',1) 
		#print "KAMRAN", fd.tell(), self.usedefaults.fmt_vbpString , sz, usefmt, fill_style
		...
		self.pdfOut.addPolyLine(polygon=1, stroke=color_line, strokewidth=sw, points=upoints, fill_style=fill_style, edge_visible = edge_visible, 
				color_fill = color_fill,  strokedasharray=self.usedefaults.parameters['stroke_dasharray'],patternIndex=str(patternIndex));
	...
	def c5_32(self,record,fd): 
		# KAMRAN HUSAIN 
		patternIndex = record.readInt(fd) 
		nx = record.readInt(fd) 
		ny = record.readInt(fd) 
		precision =  record.readInt(fd) 
		fsize = nx * ny * 3; 
		datafmt  = "%s%dB" % (self.usedefaults.os_prelude, fsize)
		#print "c5_32 size = ", patternIndex,nx,ny,precision,fsize, datafmt
		binaryData   =  fd.read(fsize) 
		#print "Size of data ...",  (nx,ny), len(binaryData) 
		imageData = Image.fromstring('RGB',(nx,ny),binaryData)
		myIO = StringIO.StringIO()           
		imageData.save(myIO,'PNG')
		encodedData = myIO.getvalue().encode('base64') 
		myIO.close() 	
		print "Binary data=", encodedData[:32], nx, ny, precision, patternIndex
		self.pdfOut.addPattern(patternIndex, nx, ny, precision, encodedData) 
	...
	def processCommand(self,fd,record):
		try :
			...
			print "EXCEPTION or BAD OPCODE", record.elementClass, record.elementID, record.parmslen , fd.tell() 
			self.dummyReader(fd,record)
...
if __name__ == '__main__':
	...
	xx.readFile(inputfilename,outputfilename,xx)
	# ----------------------------------------------------------------
